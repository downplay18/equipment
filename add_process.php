...
if (!isset($_POST['var_lastSuffix'])) {
    array_push($_SESSION['addMsg'], "<p style='font-size:1.5em;color:red;'>ERROR! เลือกชื่อเครื่องมือเครื่องใช้จากในลิสต์เท่านั้น</p>");
...
if (empty($_FILES['fileToUpload']['name'])) {
    array_push($_SESSION['addMsg'], "ไม่พบไฟล์ใบเสร็จ...");
    $uploadOk = 0;
...
// เช็คไฟล์ซํ้า
    if (file_exists($target_file)) {
        array_push($_SESSION['addMsg'], "<p style='font-size:1.5em;color:red;'>พบชื่อไฟล์ใบเสร็จซํ้ากัน โปรดติดต่อผู้ดูแลระบบ!</p>");
...
// Check file size
    ...
    if ($_FILES["fileToUpload"]["size"] > $maxFileSize) {
        ...
        array_push($_SESSION['addMsg'], "<p style='font-size:1.5em;color:red;'>ไม่อนุญาตให้อัปโหลดไฟล์ใบเสร็จที่มีขนาดเกิน 10MB!</p>");
...
// เช็คนามสกุลไฟล์
    ...
    if ($imageFileType != "pdf" && $imageFileType != "doc" && $imageFileType != "docx" && $imageFileType != "jpg" && $imageFileType != "jpeg" && $imageFileType != "png" && $imageFileType != "gif") {
        array_push($_SESSION['addMsg'], "<p style='font-size:1.5em;color:red;'>นามสกุลไฟล์ไม่ถูกต้อง รองรับเฉพาะ pdf, doc, docx, jpg, jpeg, png, gif เท่านั้น</p>");
        $uploadOk = 0;
    } else {
...
for ($rc = 0; $rc < $row_count; $rc++) { /* 1 $rc คือ 1 แถวของรายการใน 1 ใบเสร็จ */
    $slipQS .= "INSERT INTO `item_slip` (`zpo`,`slip_date`,`detail`,`slip_suffix`,`qty`,`unit_price`,`amount`,`sub_total`,`grand_total`,`adder`)";
    ...
    $slipQS .= ",'" . $postUnitPrice[$rc] . "'"; /* unit_price[] */
    $slipQS .= ",'" . $postAmount[$rc] . "'"; /* amount[] */
    $slipQS .= ",'" . $_POST['var_subTotal'] . "'"; /* sub_total */
    $slipQS .= ",'" . $_POST['var_grandTotal'] . "'"; /* grand_total */
    ...
    $addRecordQS .= ",'" . $target_file . "'"; //slip
    $addRecordQS .= ",'" . $_POST['var_slipDate'] . "'"; //slip
    $addRecordQS .= ");";
    ...
    $userAlert = $rc + 1;
    array_push($_SESSION['addMsg'], "เพิ่มเบิกคลัง#$userAlert ...OK!");
}
...
if ($fullQry) {
    array_push($_SESSION['addMsg'], "<p style='font-size:1.25em;color:blue;'>-- เพิ่ม $row_count รายการเสร็จสิ้น --</p>");
}
?>
...
if ($uploadOk == 1 && move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
    array_push($_SESSION['addMsg'], "อัปโหลดไฟล์ " . basename($_FILES["fileToUpload"]["name"]) . " เสร็จสมบูรณ์!");
    array_push($_SESSION['addMsg'], "<a href=\"$target_file\" target=\"_blank\" style='font-size:1.25em;color:blue;'>คลิกที่นี่เพื่อดูใบเสร็จ</a>");
} else {
    array_push($_SESSION['addMsg'], "ไม่มีการอัปโหลดใบเสร็จ(" . $uploadOk . ")");
}
?>
...
<div class="container-fluid">
    ...
    ?>
    <br/>
    <a href='add.php' class='btn btn-danger btn-lg' style="margin:1em"><span class='glyphicon glyphicon-arrow-left'></span> กลับ</a>
</div>
